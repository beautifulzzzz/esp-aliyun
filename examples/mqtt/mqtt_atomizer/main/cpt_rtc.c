/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2019年08月03日 星期六 10时48分40秒
 ************************************************************************/
#include "cpt_rtc.h"


u8 cpt_rtc_init(cpt_rtc_params_s *param);
void cpt_rtc_set_date(cpt_rtc_date_s date);
void cpt_rtc_get_date(cpt_rtc_date_s *date);
void cpt_rtc_date_to_utc(cpt_rtc_date_s date, u32 *utc_time);   
void cpt_rtc_utc_to_date(u32 utc_time, cpt_rtc_date_s *date);
u8 cpt_rtc_is_same(cpt_rtc_date_s *time_stamp1, cpt_rtc_date_s *time_stamp2, cpt_rtc_cmp_kind_s kind);
void cpt_rtc_run(void);
...
cpt_rtc_s cpt_rtc = {
    .init = &cpt_rtc_init,
    .set_date = &cpt_rtc_set_date,
    .get_date = &cpt_rtc_get_date,
    .date_to_utc = &cpt_rtc_date_to_utc,
    .utc_to_date = &cpt_rtc_utc_to_date,
    .is_same = &cpt_rtc_is_same,
    .run = &cpt_rtc_run,
};
...
void cpt_rtc_set_date(cpt_rtc_date_s date){
    u32 cur_utc_time;
    cpt_rtc_date_to_utc(date,&cur_utc_time);
    p_this->cur_utc_time = cur_utc_time;
}

void cpt_rtc_get_date(cpt_rtc_date_s *date){
    cpt_rtc_utc_to_date(p_this->cur_utc_time,date);
...
void cpt_rtc_utc_to_date(u32 utc_time, cpt_rtc_date_s *date){
    if(utc_time < UTC_TIME2000){
        memset(date,0,sizeof(cpt_rtc_date_s));
        return;
    } 
